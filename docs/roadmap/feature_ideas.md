...
| **Create Issue (with Metadata)**  | ✅ DONE | `:TicketsCreate` opens frontmatter-based template to specify **title, description, labels, and assignees**. Submit with `<leader>s` or `<CR><CR>`. Uses scratch buffer to avoid file write complications.             |
...
| **Create Issue Refinements**      | 💡 Ideas | <ul><li>Add milestone support in frontmatter</li><li>Template persistence/customization per repo</li><li>Issue templates from `.github/ISSUE_TEMPLATE/`</li><li>Preview rendering before submission</li><li>Draft issues (save locally before submitting)</li></ul> |
